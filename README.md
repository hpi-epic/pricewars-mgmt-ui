...
Lastly, we are also experiencing some bugs with Highcharts, that only appear infrequently though and are not easily reproducable. Eg in the price-graph when switching to another product-filter, Highcharts sometimes does not render anything and also misplaces the legend with an `Uncaught TypeError: Cannot set property 'pointStart' of undefined` caused by the `redraw`-function after new points have been added. Unfortunately, we were not yet able to solve this bug since there also seems to be very little documentation about it.   
